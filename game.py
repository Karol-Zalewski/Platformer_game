...
def scale_sprites(sprites_dict, width, height):
    new_sprites_dict = {}
    for sprites_name, sprites in sprites_dict.items():

        new_sprites_dict[sprites_name] = [pygame.transform.scale(
            sprite, (width, height)) for sprite in sprites]

    return new_sprites_dict
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, width, height):
        super().__init__()
        self.SPRITES = scale_sprites(self.SPRITES, width, height)
...
class Fire(Object):
    SPRITES = load_sprite_sheets("Traps", "Fire", 16, 32)
